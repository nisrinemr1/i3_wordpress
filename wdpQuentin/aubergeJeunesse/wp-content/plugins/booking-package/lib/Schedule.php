    ...
    class booking_package_schedule {
				...
				foreach ((array) $calendarList as $key => $value) {
					...
					if ($account['type'] == 'day' && intval($account['courseBool']) == 1) {
						...
						foreach ((array) $bookedRows as $bookedKey => $bookedValue) {
							...
							$options = json_decode($bookedValue['options'], true);
							for ($i = 0; $i < count($options); $i++) {
								...
								if (isset($bookedServices[$dayKey])) {
									...
									if (isset($bookedServices[$dayKey][$timeKey])) {
										...
										if (isset($bookedServices[$dayKey][$timeKey][$option['key']])) {
											
											$bookedServices[$dayKey][$timeKey][$option['key']]++;
											
										} else {
											
											$bookedServices[$dayKey][$timeKey][$option['key']] = 1;
											
										}
										
									} else {
										
										$bookedServices[$dayKey][$timeKey] = array(intval($option['key']) => 1);
		...
		public function serachCourse($accountKey, $scheduleKey, $key = false, $bookingYMD = null, $time = false){
			...
			if ($key !== false) {
				...
				$sql = $wpdb->prepare(
					"SELECT `key`, `name`, `time`, `cost`, `expirationDateStatus`, `expirationDateFrom`, `expirationDateTo`, `stopServiceUnderFollowingConditions`, `doNotStopServiceAsException` FROM `".$table_name."` WHERE `accountKey` = %d AND `key` = %d LIMIT 0, 1;", 
			...
			if ($time !== false) {
				...
				$sql = $wpdb->prepare(
					"SELECT `key`, `name`, `time`, `cost`, `expirationDateStatus`, `expirationDateFrom`, `expirationDateTo`, `stopServiceUnderFollowingConditions`, `doNotStopServiceAsException` FROM `".$table_name."` WHERE `accountKey` = %d AND `time` = %d LIMIT 0, 1;", 
			...
			if (is_null($row)) {
				...
				$invalidService = $this->invalidService($scheduleKey, $row);
		...
		public function invalidService($scheduleKey, $requestService) {
