						...
						case 0:							
							...
							if ($isMetaVar) {
								...
								if (DbWpf::query($query)) {
									...
									if ($isAllProducts && $maxCountProducts > 1000) {
										$valuesList = DbWpf::get( 'SELECT id FROM `@__meta_values` WHERE key_id=' . $keyId);
										$cntValues = count($valuesList);
										if ($cntValues > 50) {
											$valuesList = DbWpf::get( 'SELECT id FROM `@__meta_values` WHERE key_id=' . $keyId);
											$bulk = 50;
											$in = '';
											$j = 0;
											$cntValues--;
											foreach ($valuesList as $k => $v) {
												$in .= $v['id'] . ',';
												$j++;
												if ($j >= $bulk || $k >= $cntValues) {
													$q = $query . ' AND v.id in (' . substr($in, 0, -1) . ')';
													if ($k >= $cntValues) {
														$query = $q;
													} else {
														if (!DbWpf::query($q)) {
