...
function blossom_fashion_scripts() {	
    ...
    wp_enqueue_script( 'blossom-fashion-modal', get_template_directory_uri() . '/js' . $build . '/modal-accessibility' . $suffix . '.js', array( 'jquery' ), BLOSSOM_FASHION_THEME_VERSION, true );
...
if( ! function_exists( 'blossom_fashion_admin_scripts' ) ) :
/**
 * Enqueue admin scripts and styles.
*/
function blossom_fashion_admin_scripts(){
    wp_enqueue_style( 'blossom-fashion-admin', get_template_directory_uri() . '/inc/css/admin.css', '', BLOSSOM_FASHION_THEME_VERSION );
}
endif; 
add_action( 'admin_enqueue_scripts', 'blossom_fashion_admin_scripts' );
...
if( ! function_exists( 'blossom_fashion_admin_notice' ) ) :
/**
 * Adding admin notice
 */
function blossom_fashion_admin_notice() {
    global $pagenow;
    $theme_args      = wp_get_theme();
    $meta            = get_option( 'blossom-fashion-update-notice' );
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    <h3><?php esc_html_e( 'Congratulations!', 'blossom-fashion' ); ?></h3>
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'blossom-fashion' ), esc_html( $name ) ) ; ?></p>
                    <p><a href="<?php echo esc_url( admin_url( 'themes.php?page=blossom-fashion-getting-started' ) ); ?>" class="button button-primary" style="text-decoration: none;"><?php esc_html_e( 'Go to the getting started.', 'blossom-fashion' ); ?></a></p>
                    <p class="dismiss-link"><strong><a href="?blossom-fashion-update-notice=1"><?php esc_html_e( 'Dismiss','blossom-fashion' ); ?></a></strong></p>
                </div>
            </div>
        </div>
    <?php }
...
add_action( 'admin_notices', 'blossom_fashion_admin_notice' );

if( ! function_exists( 'blossom_fashion_ignore_admin_notice' ) ) :
/**
 * ignore notice
 */
function blossom_fashion_ignore_admin_notice() {

    if ( isset( $_GET['blossom-fashion-update-notice'] ) && $_GET['blossom-fashion-update-notice'] = '1' ) {

        update_option( 'blossom-fashion-update-notice', true );
    }
}
endif;
add_action( 'admin_init', 'blossom_fashion_ignore_admin_notice' );

if ( ! function_exists( 'blossom_fashion_get_fontawesome_ajax' ) ) :
/**
 * Return an array of all icons.
 */
function blossom_fashion_get_fontawesome_ajax() {
    // Bail if the nonce doesn't check out
    if ( ! isset( $_POST['blossom_fashion_customize_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['blossom_fashion_customize_nonce'] ), 'blossom_fashion_customize_nonce' ) ) {
        wp_die();
    }

    // Do another nonce check
    check_ajax_referer( 'blossom_fashion_customize_nonce', 'blossom_fashion_customize_nonce' );
    ...
    $fonts = blossom_fashion_get_fontawesome_list();
...
add_action( 'wp_ajax_blossom_fashion_get_fontawesome_ajax', 'blossom_fashion_get_fontawesome_ajax' );